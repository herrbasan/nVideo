...
function lookupCache(inputPath, config, cacheDir) {
...
function copyFromCache(cacheFile, outputPath) {
...
module.exports = {
    ...
    transcode(inputPath, outputPath, opts) {
        ...
        if (cacheEnabled) {
            ...
            const cacheResult = lookupCache(inputPath, cacheConfig, cacheDir);

            if (cacheResult.hit) {
                if (opts.onCacheHit) {
                    opts.onCacheHit(cacheResult.cacheFile);
                }
                copyFromCache(cacheResult.cacheFile, outputPath);
